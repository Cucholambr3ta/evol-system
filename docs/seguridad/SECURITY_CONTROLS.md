...
| SC-007 | Invariante anti-MCP              | Ningun artefacto generado por el framework puede referenciar `mcpServers`. El framework es CLI-first; MCP esta prohibido por diseno. | CI workflow + `evol-shield.py`            | Implementado | `grep -r "mcpServers" .claude/ skills/ scripts/` debe retornar 0 resultados |
