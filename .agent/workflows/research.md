...
**Mision:** Mantener Evol-DD y el proyecto activo en evolucion continua mediante investigacion
...
| :--- | :--- | :--- |
| `system` | Mejoras al framework Evol-DD: skills, agentes, workflows, scripts | Propuestas para el repo X-DD |
...
| :--- | :--- | :--- |
| I. Descubrir | Investiga GitHub (topics claude-code-skill), changelogs, papers | `evol-researcher run --scope <scope>` |
| II. Evaluar | Compatibilidad + colision + impacto (0.0-1.0) por hallazgo | Agente Researcher |
| III. Proponer | Genera `RESEARCH.md` + persiste en `research_proposals` (SQLite) | `evol-researcher.py` |
| IV. Revisar | Usuario revisa propuestas rankeadas | `evol-researcher list` |
| V. Aprobar | Usuario aprueba las relevantes (Art. 2) | `evol-researcher apply <ID>` |
| VI. Implementar | Otro agente (builder/architect) implementa lo aprobado | Pipeline Evol-DD |

## 3. Comandos

```
evol-researcher run --scope system            # mejoras al framework
evol-researcher run --scope project --topic testing
evol-researcher list --status proposed
evol-researcher apply rp_<id> --by <usuario>
```
...
> respaldo no se rankea. Las fuentes pasan por el filtro de confiabilidad de `/evol fact-check`.
...
- **Estado SQLite:** tabla `research_proposals` en `~/.evol-dd/state.db`
