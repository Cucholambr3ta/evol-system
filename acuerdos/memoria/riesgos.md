...
- **[MITIGADO] EDMS no implementado:** Fase 1+2+3 completadas: `evol_memory_store.py` + 6 subcomandos CLI + 3 hooks. Tests pasando. Pendiente: Fase 4 (4-tier consolidation), Fase 5 (causal chains en KG), Fase 6 (observabilidad). ChromaDB/LadybugDB siguen siendo opcionales — fallback BM25 funciona.
- **Dependencia en ChromaDB/LadybugDB:** Ambas son dependencias opcionales. Si no se instalan, el fallback es BM25 stdlib con busqueda local por keywords. Las 15 mejoras (RRF, consolidation, causal chains) requieren ChromaDB/LadybugDB instaladas.
