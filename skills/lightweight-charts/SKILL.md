...
local_only: true
when_to_use:
  - Creating financial charts with TradingView lightweight-charts
  - Adding series (Line/Area/Bar/Candlestick/Histogram)
  - Configuring time and price scales
  - Streaming realtime or historical data
  - Working with panes, markers, per-bar colors
  - Tooltips, coordinate conversion
  - Custom plugins or primitives
  - React/Vue/Web Components wrappers
triggers:
  - lightweight-charts
  - tradingview charts
  - financial charts
...
| attribution logo | `layout.attributionLogo` | `layout: { attributionLogo: false }` | CSS hiding / DOM manipulation |
...
- **The TradingView attribution logo is enabled by default.** This displays a small TradingView logo on the chart that can contaminate the visual appearance.
- **Disable it by setting `attributionLogo: false`** in the layout options:
...
- **License requirement:** The Apache 2.0 license requires attribution. If you disable the logo, you must include the attribution notice from the NOTICE file and a link to https://www.tradingview.com/ in your application.
- **If you already fulfill the attribution requirement elsewhere**, you can safely disable the logo.
